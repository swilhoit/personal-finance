export default function SecurityPage() {
  return (
    <div className="min-h-screen p-6 bg-gradient-to-br from-cyan-50 via-sky-50 to-teal-50 dark:from-gray-900 dark:via-cyan-950 dark:to-teal-950">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-4xl font-['Bungee'] mb-8 bg-gradient-to-r from-cyan-500 to-teal-500 bg-clip-text text-transparent">
          Security
        </h1>
        
        <div className="bg-white dark:bg-gray-900 rounded-3xl border-4 border-cyan-400 dark:border-cyan-600 p-8 shadow-2xl">
          <div className="prose dark:prose-invert max-w-none">
            <p className="text-gray-700 dark:text-gray-300 leading-relaxed">
              Your security is our top priority. We use industry-leading security measures to protect your data.
            </p>
            
            <h2 className="text-2xl font-['Bungee'] mt-8 mb-4 text-cyan-600 dark:text-cyan-400">
              Encryption
            </h2>
            <p className="text-gray-700 dark:text-gray-300 leading-relaxed">
              All data is encrypted using 256-bit AES encryption both in transit and at rest.
            </p>
            
            <h2 className="text-2xl font-['Bungee'] mt-8 mb-4 text-cyan-600 dark:text-cyan-400">
              Authentication
            </h2>
            <p className="text-gray-700 dark:text-gray-300 leading-relaxed">
              We use secure authentication methods including OAuth and multi-factor authentication options.
            </p>
            
            <h2 className="text-2xl font-['Bungee'] mt-8 mb-4 text-cyan-600 dark:text-cyan-400">
              Plaid Integration
            </h2>
            <p className="text-gray-700 dark:text-gray-300 leading-relaxed">
              Financial data is accessed through Plaid, a trusted and secure financial technology platform used by thousands of applications.
            </p>
          </div>
        </div>
      </div>
    </div>
  );
}