              ...
              {lastSync && (
                <div className="text-xs text-[#9b826f] dark:text-zinc-400">
                  <div className="flex items-center gap-1">
                    <div className={`w-2 h-2 rounded-full ${lastSyncSuccess ? 'bg-[#87a878]' : 'bg-[#c17767]'}`} />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          <div className="bg-[#f5f0e8] dark:bg-zinc-900 rounded-xl border border-[#e8dfd2] dark:border-zinc-800 p-6">
            <p className="text-xs text-[#9b826f] dark:text-zinc-400 uppercase tracking-wider mb-2">Total Balance</p>
            <p className="text-2xl font-bold">${totalBalance.toFixed(2)}</p>
            <p className="text-xs text-[#7d6754] dark:text-zinc-400 mt-1">Across all accounts</p>
          </div>
          
          <div className="bg-[#f5f0e8] dark:bg-zinc-900 rounded-xl border border-[#e8dfd2] dark:border-zinc-800 p-6">
            <p className="text-xs text-[#9b826f] dark:text-zinc-400 uppercase tracking-wider mb-2">Available</p>
            <p className="text-2xl font-bold">${totalAvailable.toFixed(2)}</p>
            <p className="text-xs text-[#7d6754] dark:text-zinc-400 mt-1">Ready to spend</p>
          </div>
          
          <div className="bg-[#f5f0e8] dark:bg-zinc-900 rounded-xl border border-[#e8dfd2] dark:border-zinc-800 p-6">
            <p className="text-xs text-[#9b826f] dark:text-zinc-400 uppercase tracking-wider mb-2">Accounts</p>
            <p className="text-2xl font-bold">{accounts?.length ?? 0}</p>
            <p className="text-xs text-[#7d6754] dark:text-zinc-400 mt-1">Connected</p>
          </div>
          
          <div className="bg-[#f5f0e8] dark:bg-zinc-900 rounded-xl border border-[#e8dfd2] dark:border-zinc-800 p-6">
            <p className="text-xs text-[#9b826f] dark:text-zinc-400 uppercase tracking-wider mb-2">Institutions</p>
            <p className="text-2xl font-bold">{Object.keys(accountsByInstitution).length}</p>
            <p className="text-xs text-[#7d6754] dark:text-zinc-400 mt-1">Linked banks</p>
          </div>
        </div>
        ...
        {Object.keys(accountsByInstitution).length === 0 ? (
          ...
          <div className="space-y-6">
            {Object.entries(accountsByInstitution).map(([institution, institutionAccounts]) => (
              <div key={institution}>
                <h2 className="text-sm font-semibold text-[#7d6754] dark:text-zinc-400 mb-3">
                  {institution}
                </h2>
                <div className="bg-[#f5f0e8] dark:bg-zinc-900 rounded-xl border border-[#e8dfd2] dark:border-zinc-800 overflow-hidden">
                  <div className="divide-y divide-[#e8dfd2] dark:divide-zinc-800">
                    {institutionAccounts.map((account) => (
                      <div key={account.account_id} className="p-4 hover:bg-[#faf8f5] dark:hover:bg-zinc-800/50 transition-colors">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-4">
                            <div className="w-10 h-10 rounded-lg bg-[#faf8f5] dark:bg-zinc-800 flex items-center justify-center">
                            ...
                            <div>
                              <div className="font-medium">
                                ...
                                {account.mask && (
                                  <span className="text-[#9b826f] dark:text-zinc-400 ml-2">•••{account.mask}</span>
                                )}
                              </div>
                              <div className="text-xs text-[#9b826f] dark:text-zinc-400 capitalize">
                        ...
                        {/* Account Actions */}
                        <div className="flex items-center gap-2 mt-3 pt-3 border-t border-[#e8dfd2] dark:border-zinc-800">
                          <button className="text-xs text-[#7a95a7] hover:text-[#6b8599] transition-colors">
                            View Transactions
                          </button>
                          ...
                          <button 
                            ...
                            className="text-xs text-[#c17767] hover:text-[#a85d4d] transition-colors"
                          >
                            Reconnect
                          </button>
                        </div>
