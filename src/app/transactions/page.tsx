        ...
        {(!txs || txs.length === 0) ? (
          ...
          <div className="space-y-6">
            {Object.entries(groupedTransactions).map(([monthYear, transactions]) => (
              <div key={monthYear}>
                <h2 className="text-sm font-semibold text-zinc-600 dark:text-zinc-400 mb-3 sticky top-0 bg-zinc-50 dark:bg-black py-2">
                  {monthYear}
                </h2>
                <div className="bg-white dark:bg-zinc-900 rounded-xl border border-zinc-200 dark:border-zinc-800 overflow-hidden">
                  <div className="divide-y divide-zinc-200 dark:divide-zinc-800">
                    {transactions?.map((t) => (
                      <div key={t.transaction_id} className="p-4 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors">
                        <div className="flex items-center justify-between gap-4">
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-3">
                              <div className={`w-2 h-2 rounded-full flex-shrink-0 ${t.amount > 0 ? 'bg-red-500' : 'bg-green-500'}`} />
                              <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-3">
                            <AutoSubmitSelect
                              ...
                              className="text-sm px-3 py-1 border border-zinc-200 dark:border-zinc-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-zinc-800"
                            >
