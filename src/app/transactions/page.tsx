          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div className="flex gap-4">
              <div className="text-right">
                <p className="text-xs text-gray-500 dark:text-zinc-400">Total Spent</p>
                <p className="text-lg font-semibold text-red-600 dark:text-red-400">
                  ${totalSpent.toFixed(2)}
                </p>
              </div>
              <div className="text-right">
                <p className="text-xs text-gray-500 dark:text-zinc-400">Total Income</p>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        {(!txs || txs.length === 0) ? (
          <div className="bg-gray-50 dark:bg-zinc-900 rounded-xl border border-gray-200 dark:border-zinc-800 p-12 text-center">
            ...
            <a href="/dashboard" className="inline-flex items-center px-4 py-2 bg-gray-900 text-white dark:bg-white dark:text-black rounded-lg hover:opacity-90 transition-opacity">
              Go to Dashboard
            </a>
          </div>
        ) : (
          <div className="space-y-6">
            {Object.entries(groupedTransactions).map(([monthYear, transactions]) => (
              <div key={monthYear}>
                <h2 className="text-sm font-semibold text-gray-600 dark:text-zinc-400 mb-3 sticky top-0 bg-white dark:bg-black py-2">
                  {monthYear}
                </h2>
                <div className="bg-gray-50 dark:bg-zinc-900 rounded-xl border border-gray-200 dark:border-zinc-800 overflow-hidden">
                  <div className="divide-y divide-[#e8dfd2] dark:divide-zinc-800">
                    {transactions?.map((t) => (
                      <div key={t.transaction_id} className="p-4 hover:bg-white dark:hover:bg-zinc-800/50 transition-colors">
                        <div className="flex items-center justify-between gap-4">
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-3">
                              <div className={`w-2 h-2 rounded-full flex-shrink-0 ${t.amount > 0 ? 'bg-red-600' : 'bg-green-600'}`} />
                              <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-3">
                            <AutoSubmitSelect
                              ...
                              className="text-sm px-3 py-1 border border-gray-300 dark:border-zinc-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-600 dark:bg-zinc-800 bg-white"
                            >
                            ...
                            <div className={`font-semibold text-right min-w-[100px] ${t.amount > 0 ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400'}`}>
