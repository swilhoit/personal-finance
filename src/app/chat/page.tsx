  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-black flex flex-col">
      ...
      <div className="flex-1 max-w-4xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 flex flex-col">
              ...
              <div className="flex flex-wrap gap-2 justify-center max-w-2xl mx-auto">
                {quickPrompts.map((prompt, index) => (
                  <button
                    ...
                    className="px-3 py-1.5 text-sm bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-lg hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors"
                  >
                    {prompt}
                  </button>
                ))}
                ...
                >
                  <div className={`max-w-[80%] ${message.role === "user" ? "order-2" : ""}`}>
                    <div className="flex items-start gap-3">
                        ...
                        >
                          {message.parts.map((part, partIndex) => {
                            ...
                            if (isToolCallPart(part)) {
                              return (
                                <div key={partIndex} className="text-xs text-zinc-500 dark:text-zinc-400 italic mt-2">
                      ...
                      {message.role === "user" && (
                        <div className="w-8 h-8 rounded-full bg-zinc-200 dark:bg-zinc-800 flex items-center justify-center text-xs font-bold flex-shrink-0">
                          You
                        </div>
                      )}
                    </div>
        ...
        {/* Input Form */}
        <form onSubmit={handleSubmit} className="border-t border-zinc-200 dark:border-zinc-800 pt-4">
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 px-4 py-3 rounded-xl border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50"
            />
            {status === "streaming" ? (
              <button
                type="button"
